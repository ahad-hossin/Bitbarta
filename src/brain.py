...
_COMPOSE_SCHEMA = {
    ...
    "properties": {
        ...
        "image_good": {"type": "boolean", "description": "true if the image carries real VISUAL value: a real photograph, a product shot/render, an app/UI screenshot, a chart/graph/diagram, a headshot, a brand logo, or a genuinely visual illustration. false if the image is essentially JUST TEXT — a statement/quote card, a 'breaking news' or press-release text graphic, a headline-on-colour card, or a screenshot that is only a block of words (these just repeat our headline and look bad; the post renders cleaner as text-only). Also false for blank/placeholder/solid-colour, broken/tiny/heavily-watermarked, or unrelated images. Logos ARE fine. true when no photo is attached."},
    },
...
Image quality (image_good): we put the photo front and centre, so it must add real VISUAL value. Set image_good=FALSE when the attached image is essentially just TEXT — a statement card, a quote graphic, a "breaking news" or press-release text image, a headline-on-colour card, or a screenshot that is only a block of words. Those merely repeat our headline and look bad; the post renders cleaner as text-only instead. Set image_good=true for a real photograph, a product photo/render, an app/UI screenshot, a chart/graph/diagram, a headshot, a brand logo (logos are fine), or a genuinely visual illustration. Also set false for a blank/placeholder/solid-colour image, a broken/tiny/heavily-watermarked image, or an image unrelated to the story. If no photo is attached, set image_good=true.
